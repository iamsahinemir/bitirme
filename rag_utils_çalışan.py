...
# ─────────────────────────────────────────────────────────────────────────────
# 🆕 Alarm rengine göre süre aralıklarını çıkaran generic fonksiyon
def answer_color_intervals(df: pd.DataFrame, color: str) -> str:
    seq = df[df['Situation'].str.upper() == color].sort_values("Timestamp")
    if seq.empty:
        return f"Makine hiç {color.lower()} alarm seviyesinde çalışmamış."
    segs, start, prev = [], seq.iloc[0]["Timestamp"], seq.iloc[0]["Timestamp"]
    for t in seq["Timestamp"].iloc[1:]:
        if (t - prev) > pd.Timedelta(minutes=1):
            segs.append((start, prev))
            start = t
        prev = t
    segs.append((start, prev))
    intervals = "; ".join(f"{s:%Y-%m-%d %H:%M}–{e:%Y-%m-%d %H:%M}" for s,e in segs)
    return intervals  # sadece aralık stringi

def answer_q_orange_intervals(df):
    return answer_color_intervals(df, "ORANGE")
def answer_q_yellow_intervals(df):
    return answer_color_intervals(df, "YELLOW")
def answer_q_green_intervals(df):
    return answer_color_intervals(df, "GREEN")
def answer_q_red_intervals(df):
    return answer_color_intervals(df, "RED")
...
qa_map.extend([
    ("Makine turuncu alarm seviyesinde çalıştığı aralıkları verebilir misin?", answer_q_orange_intervals),
    ("Makine sarı alarm seviyesinde çalıştığı aralıkları verebilir misin?",   answer_q_yellow_intervals),
    ("Makine yeşil alarm seviyesinde çalıştığı aralıkları verebilir misin?", answer_q_green_intervals),
    ("Makine kırmızı alarm seviyesinde çalıştığı aralıkları verebilir misin?",answer_q_red_intervals),
])
...
def rag_answer(
    ...
    # (0c) “çalıştığı aralıklar” sorusu
    m_int = re.search(r"çalıştığı\s+aralıklari|çalıştığı\s+aralıkları", user_q, flags=re.IGNORECASE)
    if m_int:
        col = None
        for word, code in [("yeşil","GREEN"),("sarı","YELLOW"),("turuncu","ORANGE"),("kırmızı","RED")]:
            if re.search(word, user_q, flags=re.IGNORECASE):
                col = code
                break
        if col:
            return answer_color_intervals(df, col)
    ...
    if m_aa:
        ...
        return ", ".join(str(d) for d in sorted(red_days))
    ...
    ue = embedder_q.encode([q],convert_to_numpy=True)
    ...
    for i in range(3):
        if Dq[0][i] >= threshold:
            ...
            if len(sig) == 2:
                ...
                if date is None:
                    return 'Lütfen sorunuzda bir tarih belirtin.'
                return fn(df, date)
            return fn(df)
